...
var app = angular.module('LineActivitiesApp', ['ngGrid', 'angularFileUpload', 'fundoo.services']);
...
app.controller('LineActivitiesBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    $scope.list = []
    $scope.currentObj = {}
    $scope.page = 1;
    $scope.pageInfo = {}
    ...
    function refreshDate(){
        $http.get('/LineActivities?page=' + $scope.page).success(function (data, status, headers, config) {
            /*$log.log(data)*/
            if (data.flag) {
    ...
    $scope.gridOptions = { data: 'list',
        ...
        columnDefs: [
            ...
            {field: 'sponsorId', displayName: '发起人ID'},
            {field: 'sponsorName', displayName: '发起用户名'},
            {field: 'product', displayName: '产品'},
            {field: 'originalPrice', displayName: '原价'},
            {field: 'presentPrice', displayName: '现价'},
            {field: 'sabreplayLimit', displayName: '限制刀数'},
            {field: 'theNumberHasBeenCut', displayName: '已砍刀数'},
            {field: 'comment', displayName: '活动线状态(1为该活动线已下单)',width: '250'},
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        $http.delete('/LineActivities/' + content.id).success(function(data, status, headers, config) {
                            if (data.flag) {

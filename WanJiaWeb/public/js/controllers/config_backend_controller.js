var app = angular.module('ConfigBackendApp', ['tm.pagination','ui.grid', 'ui.grid.resizeColumns','ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services','simditor']);

var checkboxTemplateConfig = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('ConfigBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    dropdownTemplateConfigTypeEnum = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.Config.typeEnum).length; i++) {
        dropdownTemplateConfigTypeEnum += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.Config.typeEnum[i] + '</option>'
    }
    dropdownTemplateConfigTypeEnum += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.Config.name, width: columnDefaultWidth, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'content', displayName: objFieldInfo.Config.content, width: columnDefaultWidth, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'typeEnum', displayName: objFieldInfo.Config.typeEnum, width: columnDefaultWidth, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateConfigTypeEnum},
        {field: 'comment', displayName: objFieldInfo.Config.comment, width: columnDefaultWidth, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
    ];
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/Config/' + content.id
    ...
    $scope.addContent = function(){
        ...
        createDialogService("/assets/html/edit_templates/config.html",{
            id: 'editor',
            title: '新增',
            scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/config?startTime='
